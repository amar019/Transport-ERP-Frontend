...
export default function BookingList() {
  ...
  const filteredBookings = useMemo(() => {
    ...
    return rawBookings.filter((b) => {
      ...
      // 2. Booking Status (status: BOOKED, CANCELLED)
      let matchStatus = true;
      if (filters.status !== "ALL") {
        matchStatus = b.status === filters.status;
      }

      // 3. Payment Status (paymentStatus: PENDING, PARTIAL, PAID, CREDIT)
      let matchPaymentStatus = true;
      if (filters.paymentStatus !== "ALL") {
        matchPaymentStatus = b.paymentStatus === filters.paymentStatus;
      }

      // 4. Collection Type (collectionType: PAID_AT_BOOKING, TO_PAY)
  ...
  // Print Entire Filtered List Handler
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 no-print">
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-xl md:text-2xl font-black text-slate-600 tracking-tight">
              Booking Management
            </h1>
            {selectedIds.length > 0 && (
              <span className="bg-orange-100 text-orange-800 border border-orange-200 text-xs font-extrabold px-2.5 py-0.5 rounded-full flex items-center gap-1">
          ...
          <p className="text-xs text-slate-500 font-semibold mt-0.5">
            Manage consignments, payments, and delivery bookings.
          </p>
        </div>
